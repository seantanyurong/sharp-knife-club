...
import NextReturnDate from "@/components/NextReturnDate"
...
export default function PurchaseForm() {
  ...
  const pricePerKnife = 10
  const pricePerRepair = 15
  const totalPrice = (knifeCount[0] * pricePerKnife) + (repairCount * pricePerRepair)
          ...
          <div className="px-4">
            <form className="grid items-start gap-6">
              <div className="grid gap-3">
                <Label htmlFor="knife-count">How many knives? {knifeCount[0]}</Label>
                <Slider
                  id="knife-count"
                  min={1}
                  max={20}
                  step={1}
                  value={knifeCount}
                  onValueChange={setKnifeCount}
                  className="w-full"
                />
              </div>
              <div className="grid gap-3">
                <div className="flex items-center gap-2">
                  <Label htmlFor="repair-count">Any large chips to repair?</Label>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <HelpCircle className="h-4 w-4 text-muted-foreground cursor-pointer" />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>We repair small chips of &lt;2.5mm in depth for free!</p>
                    </TooltipContent>
                  </Tooltip>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 text-center font-medium text-sm bg-muted rounded-md py-2">
                    {repairCount}
                  </div>
                  <Button
                    type="button"
              ...
              <div>
                ...
                <div className='text-sm'>
                  We&apos;ll collect it on: <NextPickUpDate />
                </div>
                <div className='text-sm'>
                  We&apos;ll return it by: <NextReturnDate />
                </div>
              </div>
              <Button type="submit" variant={'secondary'} className="font-black">🔪 Book Sharpening Service</Button>
            </form>
          </div>
          <DrawerFooter className="pt-2">
            <DrawerClose asChild>
              <WhatsAppLink origin='purchase'>
                <Button variant={'whatsapp'} type="submit" className='font-black w-full'>
                  <svg
                  ...
                  Questions? Message us on WhatsApp</Button>
              </WhatsAppLink>
            </DrawerClose>
          </DrawerFooter>
        </DrawerContent>
